...
`amber-gfx-shader-lab` is centered on graphics work in Dart. The technical target is: Design a Dart verification harness for shader systems, covering state machine modeling, transition tables, and failure-oriented tests.
...
The score is deterministic. The current threshold is 178. Risk has a penalty of 4, latency has a penalty of 2, and weight contributes a bonus of 3.
